...
module.exports = yeoman.Base.extend({
  ...
  prompting: function() {
    ...
    const prompts = [{
      ...
      name: 'layoutXml',
      message: 'How are you calling your activity xml (example : activity_home)? DONT WRITE .xml',
      store: true,
      ...
      name: 'menuXml',
      message: 'How are you calling your menu xml (this will be the bottom navigation items, example:  bottom_menu_navigation_home) ',
      store: true,
      default: this.menuXml,
      validate: function(input) {
        if (/^([a-zA-Z0-9_]*)$/.test(input)) {
          return true;
        }
        return 'Your application name cannot contain special characters or a blank space, using the default name instead : ' + this.menuXml;
      }
    }];
    return this.prompt(prompts).then(props => {
      ...
      this.props.menuXml = props.menuXml;
    });
  },
  writing: function() {
    var menuXml = this.props.menuXml;
    ...
    this.fs.copyTpl(
      this.templatePath('template_menu.xml'),
      this.destinationPath('app/src/main/res/menu/' + menuXml + '.xml'), {}
    );
    this.fs.copyTpl(
      this.templatePath('template_selector.xml'),
      this.destinationPath('app/src/main/res/color/' + menuXml + '_selector.xml'), {}
    );
    this.fs.copy(
